...
parser.add_argument('--anomaly_batch_size', type=int, default=32) #increase to 64 if available
...
def run_exps():
    ...
    for subset_r in tqdm(subset):
        ...
        if preprocessed:
            train_file = f"../datasets/web_attack_full/train/{subset_r}.txt"
            test_file = f"../datasets/web_attack_full/test/{subset_r}.txt"
            outlier_file = f"../datasets/web_attack_full/test/{subset_r}-outliers.txt"
        if contamination != 0:
            train_file = f"../datasets/web_attack_full/train/{subset_r}-contaminated/{subset_r}_c{int(contamination)}.txt"
